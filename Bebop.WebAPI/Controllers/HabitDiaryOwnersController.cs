...
namespace WebAPI.Controllers
{
    ...
    [Route("[controller]")]
    [Authorize]
    public class HabitDiaryOwnersController : ControllerBase
    {
        ...
        public HabitDiaryOwnersController(IHttpClientFactory httpClientFactory,
        ...
        {
            var response = await _diaryServiceClient.GetAsync("/api/HabitDiaryOwner/AllDiaryOwners");
        ...
        {
            var response = await _diaryServiceClient.GetAsync($"/api/HabitDiaryOwner/GetDiaryOwner/{id}");
            return await CheckResponse(response);
        }
        ...
        public async Task<IActionResult> Add([FromBody] JsonElement json)
        {
            ...
            var response = await _diaryServiceClient.PostAsync($"/api/HabitDiaryOwner/CreateDiaryOwner", JsonContent.Create(json));
            return await CheckResponse(response);
        }
        ...
        public async Task<IActionResult> Update([FromBody] JsonElement json, Guid id)
        {
            var token = User.Claims.GenerateJwtToken(_validApiKeys);
            _diaryServiceClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            var response = await _diaryServiceClient.PutAsync($"/api/HabitDiaryOwner/UpdateDiaryOwner/{id}", JsonContent.Create(json));
            return await CheckResponse(response);
        }
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            var token = User.Claims.GenerateJwtToken(_validApiKeys);
            _diaryServiceClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            var response = await _diaryServiceClient.DeleteAsync($"/api/HabitDiaryOwner/DeleteDiaryOwner/{id}");
            return await CheckResponse(response);
        }
