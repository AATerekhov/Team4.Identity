...
namespace WebAPI.Controllers
{
    ...
    [Route("[controller]")]
    [Authorize]
    public class RewardMagazineLinesController : ControllerBase
    {
        ...
        public RewardMagazineLinesController(IHttpClientFactory httpClientFactory,
        ...
        {
            var response = await _magazineServiceClient.GetAsync("/api/RewardMagazineLines/AllMagazineLines");
        ...
        {
            var response = await _magazineServiceClient.GetAsync($"/api/RewardMagazineLines/GetMagazineLine/{id}");
            return await CheckResponse(response);
        }


        [HttpGet("{id:guid}")]
        public async Task<IActionResult> GetLinesByMagazineIdAsync(Guid id)
        {
            var response = await _magazineServiceClient.GetAsync($"/api/RewardMagazineLines/GetMagazineLinesByMagazineId/{id}");
            return await CheckResponse(response);
        }
        ...
        public async Task<IActionResult> Add([FromBody] JsonElement json)
        {
            ...
            var response = await _magazineServiceClient.PostAsync($"/api/RewardMagazineLines/CreateMagazineLine", JsonContent.Create(json));
            return await CheckResponse(response);
        }
        ...
        public async Task<IActionResult> Update([FromBody] JsonElement json, Guid id)
        {
            var token = User.Claims.GenerateJwtToken(_validApiKeys);
            _magazineServiceClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            var response = await _magazineServiceClient.PutAsync($"/api/RewardMagazineLines/UpdateMagazineLine/{id}", JsonContent.Create(json));
            return await CheckResponse(response);
        }
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            var token = User.Claims.GenerateJwtToken(_validApiKeys);
            _magazineServiceClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            var response = await _magazineServiceClient.DeleteAsync($"/api/RewardMagazineLines/DeleteMagazineLine/{id}");
            return await CheckResponse(response);
        }
