...
namespace WebAPI.Controllers
{
    ...
    [Route("[controller]")]
    [Authorize]
    public class RewardMagazineOwnersController : ControllerBase
    {
        ...
        public RewardMagazineOwnersController(IHttpClientFactory httpClientFactory,
        ...
        {
            var response = await _magazineServiceClient.GetAsync("/api/RewardMagazineOwner/AllMagazineOwners"); 
        ...
        {
            var response = await _magazineServiceClient.GetAsync($"/api/RewardMagazineOwner/GetMagazineOwner/{id}");
            return await CheckResponse(response);
        }
        ...
        public async Task<IActionResult> Add([FromBody] JsonElement json)
        {
            ...
            var response = await _magazineServiceClient.PostAsync($"/api/RewardMagazineOwner/CreateMagazineOwner", JsonContent.Create(json));
            return await CheckResponse(response);
        }
        ...
        public async Task<IActionResult> Update([FromBody] JsonElement json, Guid id)
        {
            var token = User.Claims.GenerateJwtToken(_validApiKeys);
            _magazineServiceClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            var response = await _magazineServiceClient.PutAsync($"/api/RewardMagazineOwner/UpdateMagazineOwner/{id}", JsonContent.Create(json));
            return await CheckResponse(response);
        }
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            var token = User.Claims.GenerateJwtToken(_validApiKeys);
            _magazineServiceClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            var response = await _magazineServiceClient.DeleteAsync($"/api/RewardMagazineOwner/DeleteMagazineOwner/{id}");
            return await CheckResponse(response);
        }
